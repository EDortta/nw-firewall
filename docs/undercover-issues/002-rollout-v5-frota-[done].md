...
- [x] **Pré-req 3 — broker MQTT:** validação operacional (não-código), pendente.
...
- **AUTHMON_API_KEY por-nó:** descoberta que a chave é por-LB, não compartilhada —
  fonte de verdade em `config-agent/config/credentials.json` → `authmon_keys`.
  Nova chave gerada para `mgmt01atl` (master não precisa servir `:8741`).
  Chaves dos LBs injetadas via `sed` no `/etc/authmon/env` após cada `upload-tool`.
- **mgmt01atl:** código novo deployado, agent ativo, `authmon-api` desabilitado
  (master não serve Border API — config-agent só chama os LBs).
- **lb01sp:** cutover manual controlado (venv + unit copiados sem gap, v4 morto +
  `authmon-api` v5 subido, validado `/v5/health=200`). v4 + código removidos.
- **lb01atl:** mesma receita; unit legado (`/usr/bin/python3`) substituído pelo
  correto (`/opt/authmon-venv`), venv criado, `/v5/health=200`.
- **Demais nós** (sp01-03, stg01-02, db01sp-02, atlanta01-02, log-sp, web01sp,
  web01atl, apresentacao, sbox1): `upload-tool firewall` em paralelo; agent ativo,
  código novo (24 refs `target_node`), cron v4 = 0.
- **testes-internos** (192.168.7.200): IP local/privado, não alcançável pela
  infra remota — excluído.

## Critérios de aceite

- [x] `authmon-agent.service` ativo e sem restart em todos os nós alvo.
- [x] **Nenhum** vestígio do v4 (cron limpo, `listen-to-mosquitto.py`/`api.py` v4 parados) em todos os nós.
- [x] Border API `:8741` servida pelo `authmon-api.service` v5 nos LBs; config-agent comunica normalmente.
- [x] Broker MQTT acessível; grade propaga block/unblock entre nós.
- [x] `/var/log/nw-monitor.log` para de crescer com `Connection refused`.
